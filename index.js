...
function getSettings() {
    ...
    // 清理孤儿绑定：角色被删除或改名后，旧 key 不再对应任何现有角色
    // 仅在角色列表已加载（非空）时执行，避免启动阶段 ctx.characters 为空数组导致误删所有绑定
    if (ctx.characters && Array.isArray(ctx.characters) && ctx.characters.length > 0) {
        const validKeys = new Set();
        for (const char of ctx.characters) {
            if (char) {
                validKeys.add(char.avatar || char.name);
            }
        }
        let cleaned = false;
        for (const key of Object.keys(s.bindings)) {
        ...
        if (cleaned) {
            console.log(`[${DISPLAY_NAME}] Cleaned up orphaned bindings`);
            saveSettings();
        }
    }
    ...
    <div id="csync-settings" class="csync-settings">
        <div class="inline-drawer">
            ...
            <div class="inline-drawer-content">
                <div class="csync-desc">切换角色时自动同步切换主题、背景和 API 连接。使用前请点击刷新列表获取角色卡</div>
                ...
                <div class="csync-actions">
                    ...
                    <button id="csync-reset-btn" class="csync-btn csync-btn-reset">一键重置</button>
                </div>
                ...
                <div class="csync-table-wrapper">
                    ...
                    ${characters.length === 0 ? '<p class="csync-empty">角色列表尚未加载，请点击「刷新列表」获取角色卡。</p>' : ''}
                </div>
            </div>
        </div>
    </div>`;
...
function bindSettingsEvents() {
    ...
    $('#csync-clear-all-btn').off('click').on('click', function () {
        if (confirm('确定要清除所有角色的绑定吗？此操作不可撤销。')) {
            clearAllBindings();
        }
    });
    $('#csync-reset-btn').off('click').on('click', function () {
        if (confirm('确定要一键重置所有设置吗？这将恢复所有开关为默认值，并清除所有角色的绑定。此操作不可撤销。')) {
            resetAllSettings();
        }
    });
}
...
function clearAllBindings() {
    ...
    // 直接更新当前 UI 中的行状态，避免完整刷新
    document.querySelectorAll('.csync-char-row').forEach(row => {
        row.querySelectorAll('select').forEach(s => s.value = '');
        row.classList.remove('csync-has-binding');
        const dot = row.querySelector('.csync-binding-dot');
        if (dot) dot.style.display = 'none';
    ...
    console.log(`[${DISPLAY_NAME}] All bindings cleared`);
}
...
function resetAllSettings() {
    ...
    console.log(`[${DISPLAY_NAME}] All settings reset to defaults`);
}
...
async function refreshSettingsPanel() {
    ...
    // 保存当前抽屉展开状态（down 类存在表示折叠，不存在表示展开）
    const icon = container.querySelector('.inline-drawer-icon');
    const wasOpen = icon && !icon.classList.contains('down');
...
let _csyncMounting = false;  // 防重复挂载

function init() {
    console.log(`[${DISPLAY_NAME}] v${EXTENSION_VERSION} Initializing...`);
    tryMountPanel();
    ...
    if (target) {
        const observer = new MutationObserver(() => {
            if (!document.getElementById('csync-settings') && !_csyncMounting) {
                tryMountPanel();
            }
        });
...
async function tryMountPanel() {
    // 防重复挂载：如果已在挂载中或已存在，直接返回
    if (_csyncMounting) {
        console.log(`[${DISPLAY_NAME}] Mount already in progress, skipping`);
        return;
    }
    if (document.getElementById('csync-settings')) {
        return;
